...
function statusFor(meeting, remaining) {
  ...
  if (!meeting.running) return { label: 'En pause', tone: 'paused' }
  if (remaining < 0) return { label: 'Dépassement', tone: 'alert' }
  return { label: 'En cours', tone: 'running' }
}

export default function App() {
  const { meeting, configure, toggle, next, reset, rename } = useMeetingTimer()
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      if (event.key === 'Enter' && !meeting.finished) next()
    }
    document.addEventListener('keydown', handleKeyDown)
    return () => document.removeEventListener('keydown', handleKeyDown)
  }, [meeting.finished, next, toggle])
  ...
  return (
    <div className="app-shell" id="top">
      ...
      <main>
        ...
        <SolarTimeline meeting={meeting} remaining={remaining} onRename={rename} />
        <Controls meeting={meeting} onToggle={toggle} onNext={next} onReset={reset} />
      </main>
      <footer><span>MicadoTimer</span><span>Le temps partagé, sans perdre le fil.</span><span className="shortcuts">Espace · pause &nbsp; Entrée · suivant</span></footer>
    </div>
  )
}
