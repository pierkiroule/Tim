...
export function Dashboard({ meeting, meetingTime, subjectTime, impact }) {
  ...
  return (
    <section className="dashboard" aria-label="Indicateurs de la réunion">
      <article className="dashboard-global"><span>Temps global restant</span><strong className={meetingTime < 0 ? 'negative' : ''}>{formatTime(meetingTime, { signed: true })}</strong><small>sur {formatTime(meeting.totalSeconds)}</small></article>
